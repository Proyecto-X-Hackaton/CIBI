...
> - **🟢 CIBI (teléfono, siempre)**: `BERGAMOT_ES_EN`/`BERGAMOT_PT_EN` + `VISIONPSY_NANO_460M_MULTIMODAL_Q8_0` (+mmproj pareado, `image_no_upscale:'on'`) + `OCR_LATIN` + `HEALTHCARE_1_7B_MEDICAL_Q4_K_M` (= MedPsy-1.7B Q4_K_M) + `GTE_LARGE_FP16`. Rápido, offline, el del video. (Constantes verificadas SDK 0.19.0; tabla canónica en TECH_STACK §4.)
> - **🔵 CIBI Pro (peer QVAC, P1 non-judged)**: `HEALTHCARE_4B_MEDICAL_Q4_K_M` (= MedPsy-4B, 2.7GB) + Qwen3.5-4B GGUF (oficial Alibaba feb-2026, 4B multimodal, Apache-2.0). *Gate: pinnear fichero GGUF exacto + ctx probado + `qvac doctor` en peer.*
